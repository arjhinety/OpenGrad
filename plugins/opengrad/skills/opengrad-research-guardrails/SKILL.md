...
- **Amendments are recorded twice**, in `03-PREREGISTRATION.md` and in `reports/ERRATA.md`, in the same commit;
  `study_002_prereg_v7` was recorded only once and needed a late entry (ERRATA §18). A draft amendment is
  recorded in neither until the owner adopts it; `study_002_prereg_v8` (40) was drafted, then adopted the
  same day and appended at the end of 03 so that cited line numbers held (ERRATA §24).
