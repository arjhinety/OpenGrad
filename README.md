...
[![Research status](https://img.shields.io/badge/research-Study%20001%20frozen%20%7C%20Study%20002%20pre--registration-052B42?style=flat-square)](reports/M1_DPO_EVALUATION.md)
...
| Studies | **Study 001 frozen 2026-09-13** at tag [`study-001`](https://github.com/arjhinety/OpenGrad/tree/study-001). **Study 002 stopped before training (2026-09-25), no model result:** it was to explain the refusal/direct-answer regression before trying to correct it. Its preregistered stop rule 2 fired when the unanswerable set `P-UNANS` could not reach its labeller-agreement floor (0.770 against 0.80). A second `P-UNANS` attempt is drafted ([44](docs/research/study-002/44-PUNANS-V2-AMENDMENT-DRAFT.md)) and awaits the owner's adoption. The four-mode confirmatory set `P-CONF-v1` was built ([negative result](reports/study-002/punans-v1/NEGATIVE-RESULT.md), [current state](docs/research/study-002/README.md#current-state)). On-policy distillation is deferred to Study 003, speculative decoding to Study 004. See [`docs/research/STUDIES.md`](docs/research/STUDIES.md) |
...
| Look up a term (B0, M0, P-DET, C1, G1–G17, frozen, gold…) | [Glossary](docs/GLOSSARY.md) |
...
```bash
uv run opengrad status                                  # repository and run state as JSON (long; pipe to a file)
...
| **M2** | On-policy distillation | — | **Not executed** in Study 001 (mock-only scaffold); deferred to Study 003 |
...
call_f1; 7 more of 453 gold calls recalled; single seed) is within noise.
...
| Systems | Latency, throughput, memory, spec decoding | **One run** — an H200 vLLM throughput and latency microsuite ([report](reports/H200_BENCHMARK_RUN.md)); memory, device and speculative decoding planned |
