# Prose classifier v2 development sets: model annotation procedure

These are the complete instructions given to each Claude subagent that labels a development or check set of first
replies for `prose-decision-classifier-v2`. Each subagent receives this text verbatim, followed by one line naming its
batch file. The file is pinned by SHA-256 in each set's task config (`model_annotators`). It is authorized by
`docs/research/study-002/37-PROSE-DECISION-CLASSIFIER-V2-DEVELOPMENT-PLAN.md`, and is the procedure of
`prose-classifier-dev-v1` plus the first-reply instruction of 36 §3.7 (the first paragraph under "How to decide"). These labels
