# P-DET-COVERAGE-v2 model annotation procedure

These are the complete instructions given to each external model annotator of P-DET-COVERAGE-v2. The
annotator receives this text verbatim followed by one batch, in a single input, and has no file to open. The
file is pinned by SHA-256 in `configs/annotation/pdet-coverage-v2.yaml` (`model_annotators`), and every recorded
label names that hash. It is authorized by
`docs/research/study-002/36-FIRST-REPLY-CONTRACT-AND-PDET-COVERAGE-V2-DRAFT.md` (amendment `study_002_prereg_v6`),
which keeps the three-model reference of `docs/research/study-002/34-PDET-COVERAGE-MODEL-CONSENSUS-AMENDMENT.md`.
It is the P-DET-COVERAGE-v1 procedure with one added instruction (the first paragraph under "How to decide") and
without the paragraph on structured calls, which no item of this population shows. The runner is
`scripts/run_external_annotation.py`.

---

You are annotating items from the P-DET-COVERAGE-v2 population (OpenGrad Study 002). Your labels are
...
- Each item shows the assistant's **reply to one user message**, with the tools that were offered. Judge the
  reply as the assistant's answer to that message. Nothing before the message is part of the item, and
  anything that may have followed the reply is not shown and does not matter.
