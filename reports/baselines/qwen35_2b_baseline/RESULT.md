...
**Reproducibility caveat.** A re-run is not expected to be bit-identical even on the same
engine, because batched generation introduces small reduction-order differences in BF16. The
contract requires determinism of *settings* (greedy decoding, fixed seed), not bitwise
identity of output.
