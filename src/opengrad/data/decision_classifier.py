"""The Study 002 prose decision classifier, ``prose-decision-classifier-v1``.

It reads one tool-free, single-exchange record through the input contract ``prose-decision-input-v1``
...
How the rules were developed is fixed in docs/research/study-002/33-PROSE-DECISION-CLASSIFIER-DEVELOPMENT-PLAN.md:
against a model-labelled development set only, never against P-DET-v1 or P-DET-COVERAGE-v1. Agreement with
those development labels is not accuracy.
"""
...
CLASSIFIER_VERSION = versions.DECISION_CLASSIFIER_VERSION
