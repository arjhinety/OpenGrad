...
margin** -- twice the Wilson half-width at the worst case `p = 0.5`, so the figure is a function of
`n` alone and cannot be flattered by an observed rate near 0 or 1. A comparison whose observed margin
is smaller than its resolvable margin is `WITHIN_NOISE`.
...
def worst_case_half_width(n: int) -> float:
    """Wilson half-width at `p = 0.5`: `z * sqrt(0.25 / n)`, a function of `n` alone."""
    if n <= 0:
        return math.inf
...
def resolvable_margin(n: int) -> float:
    """`2 x` the worst-case Wilson half-width -- the smallest difference a row of size `n` can resolve."""
...
def resolvable_row(n: int, observed_margin: float | None = None) -> dict[str, object]:
    ...
    within_noise = (
        ...
        and float(observed_margin) < margin
    )
    return {
        "n": int(n),
