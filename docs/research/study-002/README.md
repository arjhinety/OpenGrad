...
| **Next step** | **Awaiting the study owner's decision** ([35](35-OWNER-DECISIONS-AFTER-CLASSIFIER-V1-TEST.md) §4–§6). The route chosen after the v1 test (classifier v2 plus a new untouched population with at least 50 reference DIRECT items) cannot be supplied from single-exchange records: about 22 DIRECT are expected in the unused pool. Study 001's own corpus holds thousands of direct answers, but inside multi-turn conversations, which the input contract (32) and every population exclude. P-DET-v1 stays unfrozen. The owner annotates nothing. Nothing authorises C1, a mixture change or training. |
