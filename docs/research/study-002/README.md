...
| **Next step** | Decided by the study owner on 2026-09-17 after the v1 test ([35](35-OWNER-DECISIONS-AFTER-CLASSIFIER-V1-TEST.md)): **DIRECT is made measurable by `prose-decision-classifier-v2` and a new untouched population** sized for at least 50 reference DIRECT items. First a counts-only dry run of the eligible pool, then a population amendment, then a v2 development plan; v2 is frozen before any result on the new population. P-DET-v1 stays unfrozen (a sample re-label stays available). The owner annotates nothing. Nothing authorises C1, a mixture change or training. |
