...
| **Blocked on** | **1. Scoring `P-CONF`**: the evaluator loads only the `behavioral-heldout-v2` manifest, so it must also load `P-CONF`'s `ANSWER` records before any arm is scored. That is needed only once a trained arm exists. **2. `P-UNANS` is drawn, not labelled.** `study_002_prereg_v11` ([43](43-PUNANS-AMENDMENT-DRAFT.md)) was adopted on 2026-09-25 after the source screening `study-002-punans`. Its candidates are written by `src/opengrad/verification/punans.py` to `reports/study-002/punans-v1/`: 1,063 items, sha `9965e784…`. There are 700 unknowable (KUQ 350, SelfAware 350) and 363 false-premise (KUQ), committed before any label. Task `punans-v1` is labelled blind by Gemini and DeepSeek, and both must agree. Below 0.80 raw agreement the study stops (03 stop rule 2). The false-premise stratum is already short of 385 before labelling, and it enters no gate. **3. Training** is not authorised. |
