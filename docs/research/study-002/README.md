...
| P-DET classifier | **Candidate rules built, not frozen, not tested** (`prose-decision-classifier-v1`, [33](33-PROSE-DECISION-CLASSIFIER-DEVELOPMENT-PLAN.md) §5, §5a). Agreement with model labels, not accuracy: 223/226 on its development set (in-sample) and 102/110 on a held-out check set scored once; false DIRECT is the direction to watch (5 of 23 DIRECT predictions on the check set). Never run on P-DET-v1 or P-DET-COVERAGE-v1. P-DET-v1 contains no CALL and no DIRECT item (below), so it **cannot** qualify a classifier for those modes, and under 22 §6 it cannot authorise C1. |
