...
| P-DET classifier | **Frozen and tested once** (`prose-decision-classifier-v1`, [33](33-PROSE-DECISION-CLASSIFIER-DEVELOPMENT-PLAN.md) §8). By the preregistered rules UNSUPPORTED and CLARIFY qualify; DIRECT does not (its recall is not evaluable: 44 reference DIRECT items on P-DET-COVERAGE-v1, none on P-DET-v1) and CALL does not (not evaluable). C1 needs DIRECT and UNSUPPORTED, so **C1 is not authorised**. Coverage results rest on a three-model reference, which the owner has since allowed to count toward balancing permission, provisionally ([35](35-OWNER-DECISIONS-AFTER-CLASSIFIER-V1-TEST.md) §1); P-DET-v1 results on one unfrozen human pass. False DIRECT remains the weak spot (ToolACE 0 of 9, P-DET-v1 8 predictions with no DIRECT item). |
...
| **Next step** | **[36](36-FIRST-REPLY-CONTRACT-AND-PDET-COVERAGE-V2-DRAFT.md) adopted as `study_002_prereg_v6`** (2026-09-17). Contract `prose-decision-input-v2` is in code, and **P-DET-COVERAGE-v2 is drawn** (420 items, sha `8fa4868c…`, `--verify` PASS; 36 §7). Classifier v2 is **frozen** (tag `prose-decision-classifier-v2`, sha `47436ca9…`; 37 §7) and **tested once** on 2026-09-18 against P-DET-COVERAGE-v2's three-model reference (412/420 unanimous, no NO_CONSENSUS; 37 §8): **DIRECT, UNSUPPORTED and CLARIFY qualify**, CALL is NOT_EVALUABLE (no CALL reference label), macro F1 0.956, abstention 0.003. DIRECT balancing is permitted for glaive only; ToolACE stays NOT_EVALUABLE. Under the owner's delegated authority, **balancing permission was granted provisionally and C1 authorised** (38, `study_002_prereg_v7`), and **21's phases 3-5 are built**: all 181,433 normalization-v3 records carry a behaviour label from the frozen classifier; the decision balance is specified in 39 and materialized; **canonical-v3 exists** -- 88,056 records, 22,014 per decision (CALL/ANSWER/CLARIFY/UNSUPPORTED), 0 gate rejections, 88,056/88,056 renderable. It is **no arm's corpus** (21 phase 8 registered this, 2026-09-20), **no training run is authorised**, and no gold is frozen; 38 §5 names what withdraws the permissions. P-DET-v1 stays unfrozen. The owner annotates nothing. Nothing authorises training. |
...
| 34 | Prose decision classifier development plan — a model-labelled development set, used only for development; freeze before any test; v1 frozen and tested once (§8) | [33-PROSE-DECISION-CLASSIFIER-DEVELOPMENT-PLAN.md](33-PROSE-DECISION-CLASSIFIER-DEVELOPMENT-PLAN.md) |
