...
| **Blocked on** | **1. `P-CONF` is not built**: the `ANSWER` strata exist (above), and building the four-mode confirmatory set from them is an owner decision. **2. `P-UNANS` is not built**; the gate needs n ≥ 385 scored items (`study_002_prereg_v8` item B). **3. Training** is not authorised. |
| **Decisions needed from the owner** | (a) whether to build `P-CONF` with these strata, given that `ANSWER-natural` (n = 284) meets the floor only; (b) whether and when to authorise a training run. P-DET-v1 stays unfrozen; no gold is frozen. The `ANSWER` source was decided on 2026-09-24 ([20](20-CLOSURE-REPORT.md) open item 2). |
