...
| **Blocked on** | **1. Scoring `P-CONF`**: the evaluator loads only the `behavioral-heldout-v2` manifest, so it must also load `P-CONF`'s `ANSWER` records before any arm is scored. That is needed only once a trained arm exists. **2. `P-UNANS` is not built**; the gate needs n ≥ 385 scored items (`study_002_prereg_v8` item B). Its sources were screened on 2026-09-25 (`registry/source_screening.yaml`, `study-002-punans`; report `reports/source-screening/study-002-punans/REPORT.md`): KUQ and SelfAware are shortlisted, and the definition, source, time rule, agreement floor and tool pairing were decided by the owner on 2026-09-25 and are written into the draft amendment [43](43-PUNANS-AMENDMENT-DRAFT.md) (`study_002_prereg_v11`, not adopted; nothing drawn). The sources are KUQ and SelfAware. An unknowable stratum carries `refusal_correctness`, and a separate false-premise stratum is scored by a judge not yet built. The agreement floor is raw agreement ≥ 0.80 with κ reported, and each item is paired with tools that cannot serve it. **3. Training** is not authorised. |
| **Decisions needed from the owner** | (a) whether and when to authorise a training run. P-DET-v1 stays unfrozen; no gold is frozen. `P-CONF` was built on 2026-09-25 with both `ANSWER` strata (option A), and `ANSWER-natural` is reported as meeting the floor only. |
...
| 43 | **Draft, not adopted:** `study_002_prereg_v11`: `P-UNANS` sources (KUQ, SelfAware), definition, strata, agreement floor, tools and scoring | [43-PUNANS-AMENDMENT-DRAFT.md](43-PUNANS-AMENDMENT-DRAFT.md) |
