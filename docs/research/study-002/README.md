...
| canonical-v3 | **Sources and adapters fixed; pre-classifier `normalization-v3` built** (fingerprint `56e8abf2…`, adapter version `2.1.0` after the ToolACE repair, [31](31-CANONICAL-V3-SOURCES-AND-NORMALIZATION-V3.md)). No classifier and no behaviour label; not the training corpus. The classifier's input contract is `prose-decision-input-v1` ([32](32-CLASSIFIER-INPUT-CONTRACT.md)). |
...
| **Next step** | The study owner decides, and nothing here decides it for them: whether to re-read the 2 UNKNOWN items and freeze Pass A; and how to cover CALL and DIRECT, which P-DET-v1 cannot do (see the finding below). Nothing is frozen. `freeze-gold` locks the sessions it cites. |
...
| 31 | P-DET-COVERAGE-v1 preregistration draft — DIRECT and tool-boundary coverage (not adopted, not drawn) | [30-PDET-COVERAGE-PREREGISTRATION-DRAFT.md](30-PDET-COVERAGE-PREREGISTRATION-DRAFT.md) |
