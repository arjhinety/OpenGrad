...
| `ANSWER` strata set | **Source decided; candidates drawn; labelling in progress.** `study_002_prereg_v9` ([41](41-ANSWER-STRATA-AMENDMENT.md), owner decision 2026-09-24) names the source after a screening of 27 public candidates (`registry/source_screening.yaml`): BFCL's no-call items, relabelled (pool N), and Natural Questions questions paired with tools that cannot serve them (pool K), reported as the separate strata `ANSWER-natural` and `ANSWER-constructed`. Candidate population drawn by `src/opengrad/verification/answer_strata.py`: 1,767 items (N 917, K 850), sha `25ef4c25…`, screened against every training corpus. Task `answer-strata-v1` is labelled blind by two non-Claude models, Gemini 3.8 Flash (High) and deepseek-v4.1-flash, and the strata are the items both label `ANSWER`: `study_002_prereg_v10` ([42](42-ANSWER-STRATA-TWO-MODEL-AMENDMENT.md)) replaced 41's third model, which Codex refuses for this account, before it labelled anything. |
| **Blocked on** | **1. The `ANSWER` strata labels** (in progress, above); `P-CONF` is built after them. **2. `P-UNANS` is not built**; the gate needs n ≥ 385 scored items (`study_002_prereg_v8` item B). **3. Training** is not authorised. |
| **Decisions needed from the owner** | (a) whether and when to authorise a training run. P-DET-v1 stays unfrozen; no gold is frozen. The `ANSWER` source was decided on 2026-09-24 ([20](20-CLOSURE-REPORT.md) open item 2). |
