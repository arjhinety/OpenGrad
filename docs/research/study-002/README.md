...
| **Done since v6** | `study_002_prereg_v6` adopted ([36](36-FIRST-REPLY-CONTRACT-AND-PDET-COVERAGE-V2-DRAFT.md)); P-DET-COVERAGE-v2 drawn (420 items, sha `8fa4868c…`). Classifier v2 frozen (sha `47436ca9…`) and tested once on 2026-09-18: DIRECT, UNSUPPORTED and CLARIFY qualify, CALL is NOT_EVALUABLE, macro F1 0.956 ([37](37-PROSE-DECISION-CLASSIFIER-V2-DEVELOPMENT-PLAN.md) §8). Under `study_002_prereg_v7`, balancing is permitted provisionally (DIRECT on glaive only) and C1 is authorised ([38](38-BALANCING-PERMISSION-AND-C1-AUTHORISATION.md)). canonical-v3 is built (88,056 records, 22,014 per decision) and registered as **no arm's corpus**. The evaluation gate is executable and fails closed (`study_002_gate_v1` contract 2, 2026-09-24). Annotation is done by models only (owner decision, 2026-09-17). |
| **Blocked on** | **1. The `ANSWER` held-out stratum.** No disjoint labelled pool reaches the n ≥ 200 floor; the largest non-training candidates hold 140 and 100 `ANSWER` items ([06](06-SPLIT-SPEC.md) §C2, status note). A four-mode evaluation population cannot be built until a new held-out source exists or `UNDER_POWERED` is accepted. **2. Two undeclared preregistration values** — the truncation imbalance factor and the minimum `P-UNANS` size — without which the gate blocks ([40](40-PREREG-V8-DRAFT.md)). **3. Training** is not authorised. |
| **Decisions needed from the owner** | (a) the `ANSWER` held-out source, or a documented acceptance of `UNDER_POWERED` ([20](20-CLOSURE-REPORT.md) open item 2); (b) adopt, amend or decline the `study_002_prereg_v8` draft ([40](40-PREREG-V8-DRAFT.md)); (c) whether and when to authorise a training run. P-DET-v1 stays unfrozen; no gold is frozen. |
...
| 40 | **Draft, not adopted:** `study_002_prereg_v8` — the truncation imbalance factor and the `P-UNANS` size the gate needs, the gate on `tool_use_promotion_v6`, and wording corrections | [40-PREREG-V8-DRAFT.md](40-PREREG-V8-DRAFT.md) |
