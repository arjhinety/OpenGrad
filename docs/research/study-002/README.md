...
| **Blocked on** | **1. The `ANSWER` held-out stratum.** No disjoint labelled pool reaches the n ≥ 200 floor; the largest non-training candidates hold 140 and 100 `ANSWER` items ([06](06-SPLIT-SPEC.md) §C2, status note). A four-mode evaluation population cannot be built until a new held-out source exists or `UNDER_POWERED` is accepted. **2. `P-UNANS` is not built**; the gate needs n ≥ 385 scored items (`study_002_prereg_v8` item B). **3. Training** is not authorised. |
| **Decisions needed from the owner** | (a) the `ANSWER` held-out source, or a documented acceptance of `UNDER_POWERED` ([20](20-CLOSURE-REPORT.md) open item 2); (b) whether and when to authorise a training run. P-DET-v1 stays unfrozen; no gold is frozen. |
