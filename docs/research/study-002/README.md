...
| P-DET classifier | **Frozen, not yet tested** (`prose-decision-classifier-v1`, tag of the same name, source sha256 `64293c51…`, [33](33-PROSE-DECISION-CLASSIFIER-DEVELOPMENT-PLAN.md) §5a). Built in two rounds against model-labelled development data only. Agreement with model labels, not accuracy: 108/113 on a fresh check set scored once (false DIRECT 3 of 19 predictions). Never run on P-DET-v1 or P-DET-COVERAGE-v1; the one-shot test waits for the three-model reference (Gemini unfinished). P-DET-v1 contains no CALL and no DIRECT item (below), so it **cannot** qualify a classifier for those modes, and under 22 §6 it cannot authorise C1. |
