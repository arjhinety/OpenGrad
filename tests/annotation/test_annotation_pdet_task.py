...
from opengrad.annotation.items import load_source, project, sha256_file
...
def test_population_imports_unchanged_in_order() -> None:
    ...
    frozen = [json.loads(line) for line in POPULATION.read_text(encoding="utf-8").splitlines() if line]
    ...
    assert Counter(item.row["pdet_component"] for item in items) == {"prevalence": 400, "challenge": 181}


def test_annotators_never_see_labels_families_or_classifier_fields() -> None:
    config = load_task_config(CONFIG)
    _, items = load_source(config)
    assert any(item.row.get("challenge_families") for item in items)  # the blind list hides something real
    for item in items:
        shown = json.dumps(project(config, item.row))
        for hidden in ("challenge_families", "gold_policy_label", "classifier_version_at_selection", "annotator_rationale"):
...
def test_workspace_opens_and_exports_instrument_fields_without_touching_the_freeze(
    ...
    try:
        ...
        first = ws.item_ids[0]
        assert ws.item_view("pass-a", first)["metadata"]["pdet_component"] in {"prevalence", "challenge"}
        ws.annotate(
            "pass-a",
            first,
            {"label": "UNKNOWN", "ambiguity_status": "MISSING_CONTEXT", "annotator_rationale": "synthetic test value"},
        )
        manifest = export_snapshot(ws, ["pass-a"], tmp_path / "pkg" / "wip", exported_at="2026-09-15T00:00:00Z")
    finally:
        ws.close()
    record = json.loads((tmp_path / "pkg" / "wip" / manifest["sessions"][0]["file"]).read_text(encoding="utf-8"))
    # 23 §2: the instrument's field names, so a pass file is readable without this tool.
    for field in ("pdet_id", "gold_policy_label", "ambiguity_status", "annotator_rationale", "boundary_rule_cited", "annotator_id", "annotation_version"):
